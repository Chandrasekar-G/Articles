# Creating a MERN Boilerplate in Steps

## Step 1 : Set up the server

We'll be using the express generator for scaffolding our server code. If you do not have express generator already installed install it using the below command.

```
npm install express-generator -g
```

Create a new empty directory and use express generator to Scaffold the code structure for our server. Install all the dependencies and test if the server works fine.

```
mkdir MERN-boilerplate      # Create a new Directory
cd MERN-boilerplate         # Move to the directory
express                     # express generator for code scaffolding
npm install                 # Installing dependencies
npm start                   # Start the server
```
## Step 2 : Create React App

Now that our server code is ready, the next step is to generate the code for our react client. Similar to the express generator, there is a great command line tool called Create React App that generates the code for our react application. You can globally install it so and can use it for starting any react project.

```
npm install create-react-app -g
```

Now, in the server's root directory, we're going to create a new react app in a directory called client.
...
When we start our react dev server, It'll show us a prompt asking us if we can use a different port other since our server is also running in 3000
<img src="/boilerplates/Resources/React-Port-Prompt.png" alt="React-Port-Prompt"/>
